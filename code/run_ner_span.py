...
from optimizer.span_opt import get_optimizer
...
def evaluate(args, dev_dataloader,model, prefix=""):
    ...
    for step, batch in enumerate(dev_dataloader):
        ...
        start_labels = batch['start_positions'].cpu().numpy().tolist()
        end_labels = batch['end_positions'].cpu().numpy().tolist()

        start_pred = torch.argmax(start_logits, -1).cpu().numpy().tolist()
        end_pred = torch.argmax(end_logits, -1).cpu().numpy().tolist()
        seq_lengths = batch['attention_mask'].sum(dim=1)
        batch_size=len(start_labels)
            ...
            for i, s_l in enumerate(start_pred[k][:seq_lengths[k]]):
                ...
                for j, e_l in enumerate(end_pred[k][i:seq_lengths[k]]):
                    if s_l == e_l:
                        S.append((s_l, i, i + j))
                        break
            for i, s_l in enumerate(start_labels[k][:seq_lengths[k]]):
                if s_l == 0:
                    continue
                for j, e_l in enumerate(end_labels[k][i:seq_lengths[k]]):
                    if s_l == e_l:
                        T.append((s_l, i, i + j))
                        break
            metric.update(true_subject=T, pred_subject=S)
...
def main():
    ...
    model = model_class.from_pretrained(args.model_name_or_path,config=config,loss_type=args.loss_type)
