...
from models.bert_crf import BertCrfForNer
from processors.ner_seq import gen_dataloader,gen_test_dataloader
from processors.utils_ner import get_entities
from metrics.seq_metric import SeqEntityScore

from optimizer.crf_opt import get_optimizer
...
MODEL_CLASSES = {
    ...
    'bert': (AutoConfig, BertCrfForNer, AutoTokenizer),
}
...
def evaluate(args, dev_dataloader,model, prefix=""):
    ...
    for step, batch in enumerate(dev_dataloader):
        ...
        out_label_ids = batch['label_ids'].cpu().numpy().tolist()  #[bs,length]
        tags = tags.squeeze(0).cpu().numpy().tolist()  #[bs,length],
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        for id,tag in enumerate(tags):
            preds = tag[1:seq_lengths-1]  # [CLS]XXXX[SEP]
            label_entities = get_entities(preds, args.id2label, args.markup)
            json_d = {}
            json_d['id'] = step*args.per_gpu_eval_batch_size+id
