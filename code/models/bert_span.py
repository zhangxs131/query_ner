from transformers import BertPreTrainedModel,BertModel,ErnieModel,ErniePreTrainedModel
from transformers import ElectraModel,ElectraPreTrainedModel,NezhaModel,NezhaPreTrainedModel
...
class NezhaSpanForNer(NezhaPreTrainedModel):
    def __init__(self, config,soft_label=True,loss_type='ce'):
        super(NezhaSpanForNer, self).__init__(config)
        self.loss_type = loss_type
        self.soft_label=soft_label
        self.num_labels = config.num_labels
        if self.loss_type == 'lsr':
...
class ErnieSpanForNer(ErniePreTrainedModel):
    def __init__(self, config,soft_label=True,loss_type='ce'):
        super(ErnieSpanForNer, self).__init__(config)
        self.loss_type = loss_type
        self.soft_label=soft_label
        self.num_labels = config.num_labels
        if self.loss_type == 'lsr':
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None,end_positions=None):
        outputs = self.ernie(input_ids = input_ids,attention_mask=attention_mask,token_type_ids=token_type_ids)
        sequence_output = outputs[0]
        sequence_output = self.dropout(sequence_output)
        start_logits = self.start_fc(sequence_output)
        if start_positions is not None and self.training:
...
class ElectraSpanForNer(ElectraPreTrainedModel):
    def __init__(self, config,soft_label=True,loss_type='ce'):
